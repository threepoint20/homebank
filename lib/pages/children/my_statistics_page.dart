...
class MyStatisticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print("build");
    selected = Provider.of<AuthProvider>(context, listen: true).user;
    Provider.of<PointProvider>(context, listen: false)
        .getPointRecord(email: selected.email)
        .then((map) {
      _pointRecords = map;
      _streamController.add("update");
    });
    Provider.of<JobProvider>(context, listen: false)
        .getJobRecord(email: selected.email)
        .then((map) {
      _jobRecords = map;
      _streamController.add("update");
    });
      ...
      builder: (context, setState) {
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text('統計資料'),
            actions: [
              CircleAvatar(child: SvgPicture.string(selected.avatarSvg)),
            ],
          ),
          body: Container(
                                ...
                                Row(
                                  ...
                                  children: [
                                    Text(
                                      "Week1",
                                      style:
                                          TextStyle(color: Color(0xFF5E6773)),
                                    ),
                                    Text(
                                      "Week2",
                                      style:
